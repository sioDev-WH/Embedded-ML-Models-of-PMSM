...
# for calculating p2 (conda install -c conda-forge statsmodels)
# See https://www.statsmodels.org/0.9.0/install.html
...
def build_submodel(dfi, idx):
    ...
    dfi = dfi.assign(sin_eps_k=lambda fdf: np.sin(fdf.epsilon_k)
                     ...
                     # , cossin_idid=lambda fdf: (np.sin(fdf.epsilon_k) * fdf.id_k * np.cos(fdf.epsilon_k) * fdf.id_k)
                     # , cossin_idiq=lambda fdf: (np.sin(fdf.epsilon_k) * fdf.id_k * np.cos(fdf.epsilon_k) * fdf.iq_k)
                     # , cossin_iqiq=lambda fdf: (np.sin(fdf.epsilon_k) * fdf.iq_k * np.cos(fdf.epsilon_k) * fdf.iq_k)
    ...
    for train_idx, test_idx in cv.split(X, y):
        ...
        stat_done = False
        if ENABLE_MODEL_STATS and (not stat_done):
            # p-value and other statistics
            # https://stackoverflow.com/questions/27928275/find-p-value-significance-in-scikit-learn-linearregression
            # Xaug = smf.add_constant(X[train_idx])
            Xaug = X[train_idx]
            results_id = smf.OLS((y[train_idx])[:, 0], Xaug).fit()
            results_iq = smf.OLS((y[train_idx])[:, 1], Xaug).fit()
            print("stats for id model: ", results_id.summary())
            print("stats for iq model: ", results_iq.summary())
            stat_done = True
            # end of statistics
        ols = LinearRegression().fit(scaled_X_tr, y[train_idx])
        pred = ols.predict(scaled_X_tst)
