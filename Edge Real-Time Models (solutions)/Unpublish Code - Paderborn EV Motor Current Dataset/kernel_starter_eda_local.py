...
reduced_data = dataset.iloc[::1000, :]  # ::1000 means select rows located at n*1000 n=0,1,.... from all rows (40963202)
...
'''
dataset = dataset.assign(nk_step=lambda fdf: (fdf.n_k - fdf.n_1k),
                         sin_eps_k=lambda fdf: np.sin(fdf.epsilon_k),
...
####################
'''
We kick off regression with a linear model, as the correlation matrix suggests expedient estimation performance just 
from actual currents. Since elementary vectors are to be treated as categorical, we one-hot encode them before training.

Moreover, in order to fit in RAM, we subsample the data.

Notes:
1. We changed subsampling to 1 (removed it)
2. The purpose of one-hot is to convert categorial fields (nk and nk1) from 1...7 to (nk_1,nk_1,nk_2, ...., nk_7, nk1_1,nk1_1,nk1_2, ...., nk1_7) 
   each of which is hot or not; i.e., binary {0,1}
   See other approach in https://stackoverflow.com/questions/34007308/linear-regression-analysis-with-string-categorical-features-variables
   example:
       idx color
        0   blue
        1   green
        ...
        print(pd.get_dummies(data))
            idx        color_blue  color_green  color_red
            0           1            0          0
            1           0            1          0
            2           0            1          0
            3           0            0          1
...
'''
'''
#Let's analyze how categories are distributed.
fig, axes = plt.subplots(1, 2, sharex=True, sharey=True)
for c, ax in zip(['n_k_1', 'n_1k_1'], axes.flatten()):
    sns.countplot(x=c, data=df, palette="ch:.25", ax=ax)
unique_elem_vecs = df['n_k_1'].nunique()
'''
